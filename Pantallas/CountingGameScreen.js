...
export default function CountingGameScreen({ navigation }) {
  ...
  const sounds = useGameSounds(); const mission = MISSIONS[missionIndex % MISSIONS.length];
  ...
  const finish = () => { sounds.playWin(); setMessage('¡Misión cumplida! Semáforo en verde.'); convoyX.value = withTiming(480, { duration: 850 }); setTimeout(() => { setMissionIndex((v) => (v + 1) % MISSIONS.length); convoyX.value = -450; convoyX.value = withSpring(0); setMessage('¡Lee la misión y observa la pista!'); }, 1100); };
  const answer = (number) => { if (number === mission.answer) finish(); else { sounds.playTryAgain(); setMessage(mission.mode === 'type' ? `Cuenta solamente ${mission.target}` : 'Toca cada auto con la mirada y prueba otra vez.'); shake.value = withSequence(withTiming(-9, { duration: 70 }), withTiming(9, { duration: 70 }), withSpring(0)); } };
  return (
