...
const RACES = [
  ...
  { world: 'Semáforo', icon: '🚦', questions: [{ q: '¿Qué auto compite?', o: ['🚑', '🏎️', '🚌'], a: '🏎️' }, { q: '🔴  →  ?', o: ['✋', '▶️', '🏁'], a: '✋' }, { q: '🟢  →  ?', o: ['✋', '▶️', '🅿️'], a: '▶️' }] },
];
export default function RaceGameScreen({ navigation }) {
  const [race, setRace] = useState(0); const [question, setQuestion] = useState(0); const [message, setMessage] = useState('🏁'); const [levelComplete, setLevelComplete] = useState(false); const x = useSharedValue(0); const { playIcon } = useIconSounds();
  const stage = RACES[race]; const item = stage.questions[question]; const carStyle = useAnimatedStyle(() => ({ transform: [{ translateX: x.value }] }));
  const choose = (option) => { playIcon(option); if (option !== item.a) { setMessage('🔁'); x.value = withSpring(Math.max(0, x.value - 12)); return; } x.value = withSpring((question + 1) * 95); if (question + 1 < stage.questions.length) { setQuestion((v) => v + 1); setMessage('⭐'); } else { setMessage('🏆'); setTimeout(() => setLevelComplete(true), 700); } };
  return <GameScreen navigation={navigation} color={race === 0 ? '#F08A24' : race === 1 ? '#3DA867' : '#D64E45'} title="Copa de conocimientos" subtitle={message} score={`Etapa ${race + 1}/3`}>
    ...
    <View style={styles.track}><View style={styles.dash} /><Animated.Text accessibilityRole="button" accessibilityLabel="Escuchar auto de carrera" onPress={() => playIcon('🏎️')} style={[styles.car, carStyle]}>🏎️</Animated.Text><Text style={styles.finish}>🏁</Text></View>
    ...
    <LevelCompleteModal visible={levelComplete} game="race" level={race + 1} hasNext={race < RACES.length - 1} onNext={() => { setLevelComplete(false); setRace((v) => v + 1); setQuestion(0); x.value = 0; setMessage('🏁'); }} onReplay={() => { setLevelComplete(false); setQuestion(0); x.value = 0; setMessage('🏁'); }} onHome={() => navigation.navigate('Home')} />
  </GameScreen>;
}
