  ...
  useEffect(() => {
    if (seleccionadas.length === 2) {
      ...
      if (cards[idx1].emoji === cards[idx2].emoji) {
        setTimeout(() => sounds.playCorrect(), 650);
        // ¡Encontró una pareja!
        const nuevasCards = [...cards];
        ...
        setTimeout(() => sounds.playTryAgain(), 650);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.scoreText}>⭐ {movimientos}</Text>
      </View>

      <Text style={styles.title}>🧠</Text>

      {/* Tablero de 3 columnas x 4 filas (Ajustado para que quepa en cualquier pantalla) */}
      <Text style={styles.levelHint}>{'●'.repeat(nivel)}{'○'.repeat(3 - nivel)}</Text>
      ...
      <LevelCompleteModal visible={yaGano} game="memory" score={movimientos} level={nivel} hasNext={nivel < 3} onNext={siguienteNivel} onReplay={repetirNivel} onHome={() => navigation.navigate('Home')} />
    </SafeAreaView>
  );
}
