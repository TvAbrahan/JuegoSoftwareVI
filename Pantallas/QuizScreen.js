...
export default function QuizScreen({ navigation }) {
  const [missionIndex, setMissionIndex] = useState(0); const [message, setMessage] = useState('Escucha la misión del centro de control'); const sounds = useGameSounds();
  const mission = MISSIONS[missionIndex]; const level = missionIndex < 2 ? 1 : missionIndex < 4 ? 2 : 3;
  const choose = (vehicle) => { if (vehicle.id === mission.answer) { sounds.playCorrect(); setMessage('¡Vehículo correcto! Misión resuelta.'); setTimeout(() => { setMissionIndex((v) => (v + 1) % MISSIONS.length); setMessage('Nueva llamada del centro de control'); }, 700); } else { sounds.playTryAgain(); setMessage('Ese vehículo tiene otra misión. Intenta otra vez.'); } };
