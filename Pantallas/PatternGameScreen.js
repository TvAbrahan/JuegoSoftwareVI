...
export default function PatternGameScreen({ navigation }) {
  ...
  const drive = (vehicle) => {
    ...
    if (vehicle === mission.answer) {
      sounds.playCorrect(); setMessage(mission.mode === 'intruder' ? '¡Intruso detectado! Ruta asegurada.' : `¡Caravana lista! Viajando a ${mission.place}…`);
      roadX.value = withSequence(withSpring(-12), withTiming(500, { duration: 900 }));
      setTimeout(() => { setRoute((value) => value + 1); setChosen(null); roadX.value = -500; roadX.value = withSpring(0); setMessage('Observa el tráfico y completa la caravana'); }, 1150);
    } else {
      sounds.playTryAgain(); setMessage(mission.mode === 'intruder' ? 'Ese sí pertenece a la secuencia. Busca el diferente.' : 'Esa opción rompe el patrón. ¡Mira quién se repite!');
