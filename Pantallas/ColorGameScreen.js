...
export default function ColorGameScreen({ navigation }) {
  ...
  const finish = (text) => {
    sounds.playWin(); setMessage(text); carScale.value = withSequence(withSpring(1.08), withTiming(0, { duration: 420 }));
    setTimeout(() => {
      setMissionIndex((value) => (value + 1) % MISSIONS.length); setPaint('#D9E2EC'); setDecal(''); setMix([]); setPhase('paint'); setMessage('¡Recibimos una misión nueva!'); carScale.value = withSpring(1);
    }, 850);
  };
